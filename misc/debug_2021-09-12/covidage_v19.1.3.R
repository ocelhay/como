...
# print("seroneg:")
# print(param, n = 200)
# print(param$Value[param$Parameter=="seroneg"])
# print(param$Value[param$Parameter=="seroneg"][1])
...
parameters <- c(
  ...
  seroneg = mean(param$Value[param$Parameter=="seroneg"],na.rm=T),
...
inputs<-function(inp, run){
  ...
  return(list(si_vector=si_vector,sd_vector=sd_vector,scr_vector=scr_vector,hw_vector=hw_vector,msk_vector=msk_vector,
              ...
              cte_vector=cte_vector,q_vector=q_vector,vc_vector=vc_vector,vcp_vector=vcp_vector,isolation=isolation,distancing=distancing,
              screen=screen,cocoon=cocoon,schoolclose=schoolclose,schoolclosepartial=schoolclosepartial,workhome=workhome,
              handwash=handwash,masking=masking,quarantine=quarantine,vaccine=vaccine,vaccinep=vaccinep,travelban=travelban,
  ...
  with(as.list(c(Y, parameters)),
       {
         ...
         P <- (S+SR+E+I+R+X+Z+V+H+HC+ICU+ICUC+ICUCV+Vent+VentC+EV+ER+EVR+VR+HCICU+HCV+
                 QS+QSR+QE+QI+QR+CL+QC+QEV+QV+QER+QEVR+QVR)
         Q <- (sum(QS)+sum(QSR)+sum(QE)+sum(QI)+sum(QC)+sum(QR)+sum(QV)+sum(QER)+sum(QEVR)+sum(QEV)+sum(QVR))/sum(P)
         # print(HCICUindex)
         # print(sum(ER))
         # print(t)
         # print(I)
         ...
         pmod<-input$pmod[t*20+1]
         dmod<-input$dmod[t*20+1]
         cmod<-input$cmod[t*20+1]
         ...
         HHC<-HC+HCICU+HCV
         lam <- (1-max(hand,mask))*p*pm*seas*(contacts%*%((rho*E+(I+CL+importation)+(1-selfis_eff)*(X+HHC)+rhos*(HH))/P))+
           (1-max(hand,mask))*p*pm*seas*(1-quarantine*quarantine_eff_other)*(contact_other%*%((rho*QE+QI+QC+QEV+QEVR+QER)/P))
         # contacts under home quarantine
         lamq<-(1-max(hand,mask))*p*pm*seas*((1-quarantine_eff_home)*contact_home%*%(((1-selfis_eff)*(X+HHC+rho*QE+QI+QC++QEV+QEVR+QER))/P))+
           (1-max(hand,mask))*p*pm*seas*(1-quarantine_eff_other)*(contact_other%*%((rho*E+(I+CL+importation)+(1-selfis_eff)*(X+HHC+rho*QE+QI+QC++QEV+QEVR+QER)+rhos*(HH))/P))
         # lamq<-0
         ...
         # ODE system
         dSdt <- -S*lam-vaccinate*age_vaccine_vector*S+vac_dur*V-
           quarantine_rate*S +(1/quarantine_days)*QS+ageing%*%S-mort*S+birth
         dSRdt<- -SR*lam+omega*R-
           quarantine_rate*SR +(1/quarantine_days)*QSR+ageing%*%SR-mort*SR
         dEdt <- S*lam+SR*lam-gamma*E+ageing%*%E- vaccinate*age_vaccine_vector*E - mort*E -
         ...
         dRdt <- vac_dur_r*VR-omega*R-vaccinate*age_vaccine_vector*R-lam*sigmaR_mod*R-quarantine_rate*R+
           ...
           nusc*propo2*(1-pdeath_icu_hco*dm)*ifr[,2]*HCICU+nusc*(1-propo2)*(1-pdeath_icu_hc*dm)*ifr[,2]*HCICU+
           nu_icu*propo2*(1-dexo2*pdeath_icuo*dm)*ifr[,2]*ICU+nu_icu*(1-propo2)*(1-pdeath_icu*dm)*ifr[,2]*ICU+
           nu_icuc*propo2*(1-dexo2c*pdeath_icuco*dm)*ifr[,2]*ICUC+nu_icuc*(1-propo2)*(1-pdeath_icuc*dm)*ifr[,2]*ICUC+
           nu_vent*(1-dexv*pdeath_vent*dm)*ifr[,2]*Vent+
           nu_ventc*(1-pdeath_vent_hc*dm)*ifr[,2]*HCV+
           nu_ventc*(1-dexvc*pdeath_ventc*dm)*ifr[,2]*VentC+nu_ventc*(1-dexvc*pdeath_ventc*dm)*ifr[,2]*ICUCV 
         ...
         dQSRdt <- quarantine_rate*SR + ageing%*%QSR-mort*QSR - (1/quarantine_days)*QSR - lamq*QSR
         dQEdt <- quarantine_rate*E - gamma*QE + ageing%*%QE-mort*QE - (1/quarantine_days)*QE+lamq*QS+lamq*QSR 
         ...
         dCdt <- report*gamma*(1-age_testing_vector*ratetestE)*(1-pclin)*(1-ihr[,2])*(E+QE)+
           reportc*gamma*pclin*(1-age_testing_vector*ratetestE)*(1-ihr[,2])*(E+QE)+
           gamma*ihr[,2]*(1-critH)*(1-prob_icu)*(E+QE)+gamma*ihr[,2]*critH*reporth*(1-prob_icu)*(E+QE)+
           gamma*ihr[,2]*prob_icu*(E+QE)+ratetestI*age_testing_vector*I+ratetestC*age_testing_vector*CL+
           gamma*age_testing_vector*ratetestE*(1-ihr[,2])*E
         dCMdt<- nus*propo2*dexo2*pdeath_ho*dm*ifr[,2]*H+nus*(1-propo2)*pdeath_h*dm*ifr[,2]*H+
           nusc*report_death_HC*propo2*pdeath_hco*dm*ifr[,2]*HC+nusc*report_death_HC*(1-propo2)*pdeath_hc*dm*ifr[,2]*HC+
           nu_icu*propo2*dexo2*pdeath_icuo*dm*ifr[,2]*ICU+nu_icu*(1-propo2)*pdeath_icu*dm*ifr[,2]*ICU+
           nu_icuc*propo2*dexo2c*pdeath_icuco*dm*ifr[,2]*ICUC+nu_icuc*(1-propo2)*pdeath_icuc*dm*ifr[,2]*ICUC+
           nu_vent*dexv*pdeath_vent*dm*ifr[,2]*Vent+nu_ventc*dexvc*pdeath_ventc*dm*ifr[,2]*VentC +
           nu_ventc*dexvc*pdeath_ventc*dm*ifr[,2]*ICUCV+ nu_ventc*report_death_HC*pdeath_vent_hc*dm*ifr[,2]*HCV+
           nusc*report_death_HC*propo2*pdeath_icu_hco*dm*ifr[,2]*HCICU+
           nusc*report_death_HC*(1-propo2)*pdeath_icu_hc*dm*ifr[,2]*HCICU +
         ...
         dCMCdt <- nusc*propo2*pdeath_hco*dm*ifr[,2]*HC+nusc*(1-propo2)*pdeath_hc*dm*ifr[,2]*HC+
           nu_icuc*propo2*dexo2c*pdeath_icuco*dm*ifr[,2]*ICUC+nu_icuc*(1-propo2)*pdeath_icuc*dm*ifr[,2]*ICUC+
           nu_ventc*dexvc*pdeath_ventc*dm*ifr[,2]*VentC+nu_ventc*dexvc*pdeath_ventc*dm*ifr[,2]*ICUCV+
         ...
         dAbdt <- nui*I+nui*X+nui*CL+ 
           ...
           nu_icuc*propo2*(1-dexo2c*pdeath_icuco*dm)*ifr[,2]*ICUC+nu_icuc*(1-propo2)*(1-pdeath_icuc*dm)*ifr[,2]*ICUC+
           nu_vent*(1-dexv*pdeath_vent*dm)*ifr[,2]*Vent+
           nu_ventc*(1-dexvc*pdeath_ventc*dm)*ifr[,2]*VentC+
           nu_ventc*(1-dexvc*pdeath_ventc*dm)*ifr[,2]*ICUCV - 
           seroneg*Ab - mort*Ab + ageing%*%Ab
         
         # print(paste("QEVR",sum(QEVR)))
         
         # return the rate of change
         list(c(S=dSdt,dSRdt,dEdt,dIdt,dRdt,dXdt,dHdt,dHCdt,dCdt,dCMdt,dVdt,dQSdt,dQSRdt,dQEdt,dQIdt,dQRdt,dCLdt,dQCdt,dICUdt,dICUCdt,dICUCVdt,
...
Y<-c(initS,initSR,initE,initI,initR,initX,initH,initHC,initC,initCM,initV, initQS,initQSR, initQE, initQI, initQR, 
     initCL, initQC, initICU, initICUC, initICUCV, initVent, initVentC, initCMC,initZ, initEV, initER, initEVR, initVR, 
...
process_ode_outcome <- function(out, iterations,intv_vector){
  ...
  if(iterations>1){
    ...
    results$mean_cases<-out$mean_cases
    results$min_cases<-out$min_cases
    results$max_cases<-out$max_cases
    ...
    results$total_reported_deaths_daily_min <- (cmortality1_min)
    results$total_reported_deaths_daily_max <- (cmortality1_max)
    results$total_reported_deaths_daily_med <- rowSums(out_mean[,(CMindex+1)])      
    # print(cmortality1_max)
...
multi_runs<-function(Y,times,parameters,input,iterations,noise,confidence,fit,fit_mat){
  ...
  aux<-array(0, dim=c(length(times),37*A+1,it))
  print(dim(aux))
  ...
  if(iterations>1){
    ...
    cases1<-(rowSums(incidence)+rowSums(incidenceh))           # daily incidence cases
    cum_cases1<-colSums(incidence)+colSums(incidenceh)         # cumulative incidence cases
    day_infections1<- round(rowSums(param_vector["gamma"]*out0[,(Eindex+1)]+
                                      param_vector["gamma"]*out0[,(QEindex+1)]+
                                      param_vector["gamma"]*out0[,(EVindex+1)]))
                                      # param_vector["gamma"]*out0[,(EVRindex+1)]+
                                      # param_vector["gamma"]*out0[,(ERindex+1)]))
    ...
    # daily infections
    infections1 <- round(100*last(cumsum(day_infections1))/sum(popstruc[,2]), 1)  # proportion of the  population that has been infected at the end of the simulation
    Rt_aux1<-rep(0, length(times))
    for (w in (ceiling(1/param_vector["nui"])+1):length(times)){
      Rt_aux1[w]<-cumsum(sum(param_vector["gamma"]*out0[w,(Eindex+1)]))/cumsum(sum(param_vector["gamma"]*out0[(w-1/param_vector["nui"]),(Eindex+1)]))
      if(Rt_aux1[w] >= 7) {Rt_aux1[w]  <- NA}
    }
    results$mean_Rt<-Rt_aux1
    results$mean_cum_cases<-cum_cases1
...
# write.csv(simul_baseline,"Base_TL_v2.csv",row.names = F)
...
# write.csv(simul_interventions,"Hypothetical_TL_2_delta.csv",row.names = F)
...
## AGE DEPENDENT MORTALITY
cinc_mort_H1 <- parameters["nus"]*parameters["propo2"]*parameters["pdeath_ho"]*dexo2_hist*(out0$mean[,(Hindex+1)])+
  parameters["nus"]*(1-parameters["propo2"])*parameters["pdeath_h"]*(out0$mean[,(Hindex+1)])
cinc_mort_HC1 <- parameters["nusc"]*parameters["report_death_HC"]*parameters["propo2"]*parameters["pdeath_hco"]*(out0$mean[,(HCindex+1)])+
  parameters["nusc"]*parameters["report_death_HC"]*(1-parameters["propo2"])*parameters["pdeath_hc"]*(out0$mean[,(HCindex+1)])
cinc_mort_ICU1 <- parameters["nu_icu"]*parameters["propo2"]*parameters["pdeath_icuo"]*dexo2_hist*(out0$mean[,(ICUindex+1)])+
  parameters["nu_icu"]*(1-parameters["propo2"])*parameters["pdeath_icu"]*(out0$mean[,(ICUindex+1)])
cinc_mort_ICUC1 <- parameters["nu_icuc"]*parameters["propo2"]*parameters["pdeath_icuco"]*dexo2c_hist*(out0$mean[,(ICUCindex+1)] )+
  parameters["nu_icuc"]*(1-parameters["propo2"])*parameters["pdeath_icuc"]*(out0$mean[,(ICUCindex+1)] )
cinc_mort_Vent1  <- parameters["nu_vent"]*parameters["pdeath_vent"]*dexv_hist*(out0$mean[,(Ventindex+1)] )
cinc_mort_VentC1 <- parameters["nu_ventc"]*parameters["pdeath_ventc"]*dexvc_hist*(out0$mean[,(VentCindex+1)] )
cinc_mort_ICUCV1 <- parameters["nu_ventc"]*parameters["pdeath_ventc"]*dexvc_hist*(out0$mean[,(ICUCVindex+1)] )
cinc_mort_HCICU1 <- parameters["nusc"]*parameters["report_death_HC"]*parameters["propo2"]*parameters["pdeath_icu_hco"]*(out0$mean[,(HCICUindex+1)] )+
  parameters["nusc"]*parameters["report_death_HC"]*(1-parameters["propo2"])*parameters["pdeath_icu_hc"]*(out0$mean[,(HCICUindex+1)] )
cinc_mort_HCV1 <- parameters["nu_ventc"]*parameters["report_death_HC"]*parameters["pdeath_vent_hc"]*(out0$mean[,(HCVindex+1)] )

totage1<-as.data.frame(cinc_mort_H1+cinc_mort_HC1+cinc_mort_ICU1+cinc_mort_ICUC1+
                         cinc_mort_Vent1+cinc_mort_VentC1+cinc_mort_ICUCV1+cinc_mort_HCICU1+cinc_mort_HCV1)

basemort_H1<-(out0$mean[,(Hindex+1)])
basemort_HC1<-parameters["report_death_HC"]*(out0$mean[,(HCindex+1)])
basemort_ICU1<-(out0$mean[,(ICUindex+1)])
basemort_ICUC1<-(out0$mean[,(ICUCindex+1)])
basemort_ICUCV1<-(out0$mean[,(ICUCVindex+1)])
basemort_Vent1<-(out0$mean[,(Ventindex+1)])
basemort_VentC1<-(out0$mean[,(VentCindex+1)])
basemort_HCICU1<-parameters["report_death_HC"]*(out0$mean[,(HCICUindex+1)])
basemort_HCV1<-parameters["report_death_HC"]*(out0$mean[,(HCVindex+1)])
basemort_I<-parameters["report_natdeathI"]*(out0$mean[,(Iindex+1)])
basemort_QI<-parameters["report_natdeathI"]*(out0$mean[,(QIindex+1)])
basemort_E<-parameters["report_natdeathI"]*(out0$mean[,(Eindex+1)])
basemort_QE<-parameters["report_natdeathI"]*(out0$mean[,(QEindex+1)])
basemort_EV<-parameters["report_natdeathI"]*(out0$mean[,(EVindex+1)])
basemort_EVR<-parameters["report_natdeathI"]*(out0$mean[,(EVRindex+1)])
basemort_ER<-parameters["report_natdeathI"]*(out0$mean[,(ERindex+1)])
basemort_QEV<-parameters["report_natdeathI"]*(out0$mean[,(QEVindex+1)])
basemort_QEVR<-parameters["report_natdeathI"]*(out0$mean[,(QEVRindex+1)])
basemort_QER<-parameters["report_natdeathI"]*(out0$mean[,(QERindex+1)])
basemort_CL<-parameters["report_natdeathCL"]*(out0$mean[,(CLindex+1)])
basemort_QC<-parameters["report_natdeathCL"]*(out0$mean[,(QCindex+1)])
basemort_X<-parameters["report_natdeathCL"]*(out0$mean[,(Xindex+1)])
...
for (i in 1:dim(cinc_mort_H1)[1]) {
  for (j in 1:dim(cinc_mort_H1)[2]) {
    # print(totage1[i,j]*ifr[j,2]+totbase1[i,j]*mort[j])
    tc<-rbind(tc,c(i, j, totage1[i,j]*ifr[j,2]))
  }
}
tc<-as.data.frame(tc)
colnames(tc)<-c("Day","Age","value")
