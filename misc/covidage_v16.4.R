...
for (i in 1:length(vec)){
  pp<-parse_age_group(vec[i])
  age_group_vectors <- list(age_group_vectors, pp)
  inp$Target[i]<-i
}
  ...
  with(as.list(c(Y, parameters)),
       {
         ...
         if (schoolclose>=1){
           school<-school_eff
           schoolclose2<-as.numeric(res.comb[schoolclose,])
         }else{schoolclose2<-0}
         # print(schoolclose2)
         # print(schoolclose)
         if(distancing){
