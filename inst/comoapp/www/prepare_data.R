...
# Beds Data ----
load(file = "./www/data/beds_data.RData")
countries_beds <- beds$country

# Age dependent hosp and mort ----
mort_sever_default <- read.csv("./www/data/COVID_severe_mortality_age.csv", header = TRUE)

if (any(mort_sever_default$mortality > mort_sever_default$severity)) {
  warning("CFR > CSR for at least one age class!")
  mort_sever_default %>% mutate(severity = max(mortality, severity))
}