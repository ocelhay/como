output$plot_rr_parameters_future <- renderPlot({
  # req(interventions$valid_future_interventions)
  req(interventions$future_mat %>% filter(intervention %in% real_rr_interventions) %>% nrow() > 0)
  ...
  # Transmissibility ----
  trans <- interventions$future_mat %>%
