output$feedback_process <- renderText({
  return(
    paste0(
      ...
      if(status_app$status == "Validated Baseline" & input$tabs == "tab_modelpredictions") span(strong("Build a Package of Interventions:"), p("Select interventions to simulate, adjust parameters as desired, and 'Run Interventions'")),
      if(status_app$status == "Locked Baseline" & input$tabs == "tab_modelpredictions") p("You can re-run interventions."),
