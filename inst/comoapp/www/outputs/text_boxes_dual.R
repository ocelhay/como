    ...
    as.character(
      div(class = "n_box_baseline_total",
          conf_interval_deaths(
            simul_baseline$results$min$total_covid_deaths[which(simul_baseline$results$min$time == end_date)],
            simul_baseline$results$med$total_covid_deaths[which(simul_baseline$results$med$time == end_date)], 
            simul_baseline$results$max$total_covid_deaths[which(simul_baseline$results$max$time == end_date)]
            ),
  ...
  paste0(
    as.character(
      div(class = "n_box_interventions_total",
          ...
          conf_interval_deaths(
            simul_interventions$results$min$total_covid_deaths[which(simul_interventions$results$min$time == end_date)],
            simul_interventions$results$med$total_covid_deaths[which(simul_interventions$results$med$time == end_date)],
            simul_interventions$results$max$total_covid_deaths[which(simul_interventions$results$max$time == end_date)]
          ),
          p("Covid-19 deaths from ", input$date_range[1], " to ", strong(end_date))
      )))
})
