output$timevis_future <- renderPlot({
  input$reset_baseline
  
  dta <- interventions$baseline_mat %>%
    bind_rows(interventions$future_mat) %>%
    bind_rows(tibble(index = nrow(interventions$baseline_mat) + nrow(interventions$future_mat) + 1, 
                     intervention = "Range of simulation", 
