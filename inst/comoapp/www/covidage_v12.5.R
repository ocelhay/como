...
for (i in 1:length(times)){
  critH[i]<-min(1-fH(sum(out0[i,(Hindex+1)]))+(1-parameters["reporth"]),1)
...
previcureq0<-rowSums(out0[,(Hindex+1)])+ rowSums(out[,(ICUCindex+1)])      # requirement for beds
previcureq20<-rowSums(out0[,(ICUindex+1)])+rowSums(out[,(VentCindex+1)])    # requirement for icu
...
cinc_mort_H0  <- cumsum(rowSums(parameters["nus"]*parameters["pdeath_h"]*(out0[,(Hindex+1)]%*%ifr[,2])))
cinc_mort_HC0 <- cumsum(rowSums(parameters["nusc"]*parameters["pdeath_hc"]*(out0[,(HCindex+1)]%*%ifr[,2])))
cinc_mort_ICU0 <- cumsum(rowSums(parameters["nu_icu"]*parameters["pdeath_icu"]*out0[,(ICUindex+1)]%*%ifr[,2]))
cinc_mort_ICUC0 <- cumsum(rowSums(parameters["nu_icuc"]*parameters["pdeath_icuc"]*out0[,(ICUCindex+1)]%*%ifr[,2]))
cinc_mort_Vent0 <- cumsum(rowSums(parameters["nu_vent"]*parameters["pdeath_vent"]*out0[,(Ventindex+1)]%*%ifr[,2]))
cinc_mort_VentC0 <- cumsum(rowSums(parameters["nu_ventc"]*parameters["pdeath_ventc"]*out0[,(VentCindex+1)]%*%ifr[,2]))
base_mort_H0 <- cumsum(rowSums(out0[,(Hindex+1)]%*%mort))
base_mort_HC0 <- cumsum(rowSums(out0[,(HCindex+1)]%*%mort))
base_mort_ICU0 <- cumsum(rowSums(out0[,(ICUindex+1)]%*%mort))
base_mort_ICUC0 <- cumsum(rowSums(out0[,(ICUCindex+1)]%*%mort))
base_mort_Vent0 <- cumsum(rowSums(out0[,(Ventindex+1)]%*%mort))
base_mort_VentC0 <- cumsum(rowSums(out0[,(VentCindex+1)]%*%mort))
base_mort_S0 <- cumsum(rowSums(out0[,(Sindex+1)]%*%mort))
base_mort_E0 <- cumsum(rowSums(out0[,(Eindex+1)]%*%mort))
base_mort_I0 <- cumsum(rowSums(out0[,(Iindex+1)]%*%mort))
base_mort_CL0 <- cumsum(rowSums(out0[,(CLindex+1)]%*%mort))
base_mort_X0 <- cumsum(rowSums(out0[,(Xindex+1)]%*%mort))
base_mort_QS0 <- cumsum(rowSums(out0[,(QSindex+1)]%*%mort))
base_mort_QE0 <- cumsum(rowSums(out0[,(QEindex+1)]%*%mort))
base_mort_QI0 <- cumsum(rowSums(out0[,(QIindex+1)]%*%mort))
base_mort_QC0 <- cumsum(rowSums(out0[,(QCindex+1)]%*%mort))
base_mort_QR0 <- cumsum(rowSums(out0[,(QRindex+1)]%*%mort))
base_mort_R0 <- cumsum(rowSums(out0[,(Rindex+1)]%*%mort))

BM0<-as.data.frame(cbind(time,base_mort_S0,base_mort_E0,base_mort_I0,base_mort_CL0,base_mort_X0,base_mort_QS0,
                         base_mort_QE0,base_mort_QI0,base_mort_QC0,base_mort_QR0,base_mort_R0,
                         base_mort_H0,base_mort_HC0,base_mort_ICU0,base_mort_ICUC0,base_mort_Vent0,base_mort_VentC0))
colnames(BM0)<-c("Time","S","E","I","CL","X","QS","QE","QI","QC","QR","R",
                 "Treated: Hospital","Untreated: Hospital","Treated: ICU","Untreated: ICU",
                 "Treated: Ventilator","Untreated: Ventilator")

BM1<-as.data.frame(cbind(time,base_mort_S1,base_mort_E1,base_mort_I1,base_mort_CL1,base_mort_X1,base_mort_QS1,
                         base_mort_QE1,base_mort_QI1,base_mort_QC1,base_mort_QR1,base_mort_R1,
                         base_mort_H1,base_mort_HC1,base_mort_ICU1,base_mort_ICUC1,base_mort_Vent1,base_mort_VentC1))
colnames(BM1)<-c("Time","S","E","I","CL","X","QS","QE","QI","QC","QR","R",
                 "Treated: Hospital","Untreated: Hospital","Treated: ICU","Untreated: ICU",
                 "Treated: Ventilator","Untreated: Ventilator")
...
ymax<-max(c((cinc_mort_H0+cinc_mort_ICU0+cinc_mort_Vent0+cinc_mort_HC0+cinc_mort_ICUC0+cinc_mort_VentC0+
             base_mort_I0+base_mort_QI0+base_mort_CL0+base_mort_QC0+base_mort_X0+base_mort_S0+
               base_mort_QS0+base_mort_E0+base_mort_QE0+base_mort_QR0+base_mort_R0),(cinc_mort_H1+cinc_mort_ICU1+cinc_mort_Vent1+cinc_mort_HC1+cinc_mort_ICUC1+cinc_mort_VentC1)))
time<-as.Date(out[,"time"]+startdate)
coul=c("#047883", "#24A9E2","#051A46","#E68029", "#D63304","#D1D604")
DM0<-as.data.frame(cbind(time,base_mort_I0+base_mort_QI0,base_mort_CL0+base_mort_QC0,base_mort_X0,base_mort_S0+
                    base_mort_QS0,base_mort_E0+base_mort_QE0,base_mort_QR0+base_mort_R0,
                    cinc_mort_H0,cinc_mort_ICU0,cinc_mort_Vent0,cinc_mort_HC0,cinc_mort_ICUC0,cinc_mort_VentC0))
...
DM<-as.data.frame(cbind(time, base_mort_I1+base_mort_QI1,base_mort_CL1+base_mort_QC1,base_mort_X1,base_mort_S1+
                        base_mort_QS1,base_mort_E1+base_mort_QE1,base_mort_QR1+base_mort_R1,
                        cinc_mort_H1,cinc_mort_ICU1,cinc_mort_Vent1,cinc_mort_HC1,cinc_mort_ICUC1,cinc_mort_VentC1))
...
plot(time,Rt0,type='l',lwd=3,col='black',
     main="Baseline", xlab="Date", ylab="Reproduction number",ylim=c(0,ymax))
lines(time,Rt0/Rt0,lwd=2,col='grey')
plot(time,Rt1,type='l',lwd=3,col='black',
     main="Intervention", xlab="Date", ylab="Reproduction number",ylim=c(0,ymax))
lines(time,Rt1/Rt1,lwd=2,col='grey')
...
for (i in 1:dim(cinc_mort_H1)[1]) {
  for (j in 1:dim(cinc_mort_H1)[2]) {
    tc0<-rbind(tc0,c(i,ifr[j,1],totage0[i,j]*ifr[j,2]+totbase0[i,j]*mort[j])) 
  }
}
...
## BASELINE MORTALITY
BM1$Time<-as.Date(BM1$Time, origin = "1970-01-01")
BM0$Time<-as.Date(BM0$Time, origin = "1970-01-01")

BMF1<-melt(BM1, id.vars="Time",measure.vars = c("Treated: Hospital","Untreated: Hospital","Treated: ICU","Untreated: ICU","Treated: Ventilator","Untreated: Ventilator",
                                                "S","E","I","CL","X","QS","QE","QI","QC","QR","R"))
BMF0<-melt(BM0, id.vars="Time",measure.vars = c("Treated: Hospital","Untreated: Hospital","Treated: ICU","Untreated: ICU","Treated: Ventilator","Untreated: Ventilator",
                                                "S","E","I","CL","X","QS","QE","QI","QC","QR","R"))

mm1<-ggplot(BMF1, aes(x = Time, y = value,fill=variable))+geom_area()
mm0<-ggplot(BMF0, aes(x = Time, y = value,fill=variable))+geom_area()

ymax<-max(BMF0$value)*1.75
grid.arrange(mm0+ylab("Cumulatice mortality")+ggtitle("Baseline")+ylim(0, ymax),
             mm1+ylab("Cumulatice mortality")+ggtitle("Intervention")+ylim(0, ymax),
             nrow = 1)
...
COMPM<-as.data.frame(cbind(time,b0,b1,v0,v1))
colnames(COMPM)<-c("Days","baseline mortality: No Intervention", "baseline mortality: Intervention",
                   "reported covid-19 mortality: No Intervention", "reported covid-19 mortality: Intervention")
COMPM$Days<-as.Date(COMPM$Days, origin = "1970-01-01")
COMPM2<-melt(COMPM, id.vars="Days",measure.vars = c("baseline mortality: No Intervention", "baseline mortality: Intervention",
                                                    "reported covid-19 mortality: No Intervention", "reported covid-19 mortality: Intervention"))

ymax<-max(COMPM2$value)
cm<-ggplot(data=COMPM2,aes(x=Days,y=value,fill=variable))+
  geom_line(data=COMPM2,aes(x=Days,y=value,colour=variable),lwd=1.5)+ylim(0,ymax)+ylab("Mortality")
cm+theme_bw()
